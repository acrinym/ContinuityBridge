...
4. The pull request head has not changed while the policy is running.
5. The repository's `CI` workflow completes successfully for that exact head commit.
6. GitHub accepts a squash merge using `--match-head-commit`.

Major updates, non-SemVer updates, unexpected file changes, failed CI, cancelled CI, and stale heads are never merged automatically.
...
The metadata action is pinned to a complete commit SHA. The repository test suite also checks the workflow text for the critical no-checkout, SemVer, changed-file, CI, and head-SHA guardrails.
