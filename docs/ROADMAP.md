...
1. **User-owned continuity.** Conversation records, indexes, manifests, and generated handoffs stay under the user's control.
2. **Evidence before summaries.** Preserve original messages, provenance, and stable identifiers so another AI can inspect the source instead of trusting an opaque rewrite.
...
5. **Explicit mutation.** Import, client configuration, deletion, and live capture must be intentional and inspectable.
...
Goal: turn selected Lore evidence plus current repository coordinates into a bounded package another AI can immediately continue from.
...
Success condition: a user can hand another AI one generated file and that AI can locate the source conversations and current code state without a giant pasted transcript.

## Next product trains

### 0.6 — Safe attachment continuity

- Enumerate attachments referenced by supported exports.
...
- Handoff bundles can reference copied artifacts by relative path.
